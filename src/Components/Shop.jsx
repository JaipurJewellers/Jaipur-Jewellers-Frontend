...
function Shop() {
    ...
    const [selectedCategory, setSelectedCategory] = useState("Necklace"); // ✅ Default to Necklace
    const [allProducts, setAllProducts] = useState([]);
    const [selectedImages, setSelectedImages] = useState({});
    const [categoryImage, setCategoryImage] = useState(""); // ✅ Store first image of category
    ...
    async function fetchProducts() {
        try {
            ...
            if (response.status === 200) {
                ...
                // ✅ Update first image for the category
                if (specificProducts.length > 0) {
                    setCategoryImage(specificProducts[0]?.Image);
                } else {
                    setCategoryImage(""); // Reset if no product found
                }
            }
        } catch (error) {
    ...
    return (
        <>
            ...
            <div className='w-full h-auto flex flex-col py-10 bg-[#FAFAFA] gap-10'>
                {/* ✅ Home Section with First Image */}
                <div className='w-[90%] h-[250px] mx-auto sm:h-[350px] lg:h-[450px] 2xl:h-[600px]'>
                    {categoryImage ? (
                        <img src={categoryImage} alt="Category Image" className='w-full h-full object-cover rounded-3xl' />
                    ) : (
                        <div className="bg-gray-600 w-full h-full rounded-3xl"></div>
                    )}
                </div>
